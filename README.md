...
*   **📊 即時統計看板**
    ...
    *   **預估薪資（時薪）**：根據設定的時薪自動計算本月工時計薪收入。
    *   **獎金/其他**：來自「備忘錄／獎金紀錄」的本月獎金與其他收入加總。
    *   **本月總收入（時薪＋獎金）**：將本月時薪收入與獎金/其他加總後顯示。
...
*   **💡 獎金備忘錄與其他收入**
    *   可獨立記錄不算工時的收入，例如成交獎金、額外獎金等。
    *   每筆包含日期、金額與備註說明（例如「Altis 成交獎金」）。
    *   本月獎金小計顯示在「備忘錄／獎金紀錄」區塊與統計看板的「獎金/其他」卡片。
    *   不影響工時計算，只參與收入統計與「本月總收入（時薪＋獎金）」。
